...
class TaskUnitForRooms extends StatefulWidget {
  @override
  _TaskUnitForRoomsState createState() => _TaskUnitForRoomsState();
}

class _TaskUnitForRoomsState extends State<TaskUnitForRooms>{

  bool loadData = true;
  bool loadData2 = true;
  bool switchValue = true;
  String nameTablet = '';
  Map<String,dynamic> fkRoomsSelected = {};

  List listRoomsAll = [];
  Map<String,dynamic> allTaskUnitFunctional = {};
  Map<String,dynamic> allTaskUnitFunctionalData2 = {};
  ...
  Future initialData() async {
    ...
    nameTablet = await SharedPreferencesClass().getValue('S4CRoom') ?? '';
    String idAlojamiento = await SharedPreferencesClass().getValue('S4CAlojamientoUF') ?? '0';
    ...
    //BUSCAR LAS HABITACIONES DE LA UNIDAD FUNCIONAL
    fkRoomsSelected = {'idHabitacion' : 'UF'};
    try{
      var response = await ConnectionHttp().httpGetDataCentro(idAlojamiento: idAlojamiento);
      if(response.statusCode == 200){
        listRoomsAll = jsonDecode(response.body);
      }else{
        String errorHttp = 'Error de conexión con el servidor, no se agregaron habitaciones';
        showAlert(text: errorHttp,isSuccess: false);
      }
    }catch(e){
      showAlert(text: 'Error: ${e.toString()}',isSuccess: false);
    }
    if(listRoomsAll.isNotEmpty){
      if(mounted){
        setState(() {});
        await loadTaskUF();
        await loadTaskHabForUF();
        await loadTaskUFProgram();
        await loadTaskHabForUFProgram();
        await loadDataAll3();
      }
    }else{
      initialData();
  ...
  Future loadTaskUF()async{
    try{
      Response? response;
      response = await ConnectionHttp().httpGetDataTaskUnitFunctional();
      if(response != null && response.statusCode == 200){
        List listHttp = jsonDecode(response.body);
        allTaskUnitFunctional['UF'] = listHttp;
        setState(() {});
      }else{
  ...
  Future loadTaskHabForUF()async{
    try{
      for(int x = 0; x < listRoomsAll.length; x++){
        Response? response;
        response = await ConnectionHttp().httpGetControlesMantenimientoHabitacion(idFk: listRoomsAll[x]['idHabitacion'].toString());
        if(response != null && response.statusCode == 200){
          List listHttp = jsonDecode(response.body);
          allTaskUnitFunctional[listRoomsAll[x]['idHabitacion'].toString()] = listHttp;
          setState(() {});
        }else{
          showAlert(text: 'Error para cargar tareas asignadas',isSuccess: false);
        }
  ...
  Future loadTaskHabForUFProgram()async{
    try{
      for(int x = 0; x < listRoomsAll.length; x++){
        Response? response;
        response = await ConnectionHttp().httpGetControlesMantenimientoProgramadosHab(idFk: listRoomsAll[x]['idHabitacion'].toString());
        if(response != null && response.statusCode == 200){
          List listHttp = jsonDecode(response.body);
          allTaskUnitFunctionalData2[listRoomsAll[x]['idHabitacion'].toString()] = listHttp;
          setState(() {});
        }else{
          showAlert(text: 'Error para cargar tareas programadas',isSuccess: false);
        }
  ...
  Future loadDataAll3()async{
    ...
    String idHab = await SharedPreferencesClass().getValue('S4CfkRoom') ?? '0';
    try{
  ...
  Widget menuDerTop(){
    ...
    List<Widget> listW = [];
    listW.add(containerPatientTop(roomsData: {'idHabitacion' : 'UF'}, isUF: true));
    for(int x = 0; x < listRoomsAll.length; x++){
      listW.add(containerPatientTop(roomsData: listRoomsAll[x]));
    }
  ...
  Widget containerPatientTop({required Map<String,dynamic> roomsData, bool isUF = false}){
    String name = '';
    if(!isUF){
      if(roomsData.containsKey('stHabitacion') && roomsData['stHabitacion'].toString().isNotEmpty){
        name = roomsData['stHabitacion'];
      }else{
        return Container();
      }
    }else{
    ...
    return InkWell(
      onTap: (){
        fkRoomsSelected = roomsData;
        setState(() {});
      },
      child: Container(
        constraints: BoxConstraints(maxWidth: sizeW * 0.15),
        margin: EdgeInsets.only(right: sizeW * 0.01),
        padding: EdgeInsets.symmetric(vertical: sizeH * 0.005,horizontal: sizeW * 0.005),
        decoration: BoxDecoration(
          color: fkRoomsSelected['idHabitacion'] == roomsData['idHabitacion'] ? S4CColors().colorLoginPageBack : Colors.white,
        ...
        child: Center(
          child: Text(isUF ? name : '${name[0].toUpperCase()}${name.substring(1).toLowerCase()}',style: S4CStyles().stylePrimary(size: sizeH * 0.0225,color: S4CColors().primary,fontWeight: FontWeight.bold,),textAlign: TextAlign.center,maxLines: 2,),
        ),
      ),
    );
  }
  ...
  Widget menuDer1Container(){
    List listElements = allTaskUnitFunctional[fkRoomsSelected['idHabitacion'].toString()] ?? [];
    ...
    Container(
      ...
      child: Wrap(
        ...
        children: List.generate(listElements.length, (index){
          return menuDer1RowListContainer(element: listElements[index]);
        }),
      ),
    );
  }
  ...
  Widget menuDer2Container(){
    List listElements = allTaskUnitFunctionalData2[fkRoomsSelected['idHabitacion'].toString()] ?? [];
    ...
    Container(
      ...
      child: Wrap(
        ...
        alignment: WrapAlignment.start,
        children: List.generate(listElements.length, (index){
          return menuDer2RowListContainer(element: listElements[index]);
        }),
      ),
    );
  }
